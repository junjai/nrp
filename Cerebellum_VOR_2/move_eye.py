...
@nrp.MapVariable("V", initial_value=0.0)
...
def move_eye (t, eye_position, eye_vel, error, true_error, X, V, idx, idx_error, isSaccade, dt, eye_state):
    if dt.value is not None and error.value is not None and true_error.value is not None:
        ...
        clientLogger.info('error:', error.value[0], 'true error', true_error.value)
...
# using error from camera image or from discrepancy between eye and target position 
        ...
        if isSaccade.value == 0: # default phase
            ...
            clientLogger.info('ret: ', ret, 'idx: ', idx.value, 'X:', X.value, 'error:', error.value[0]/maxAngle)
        ...
        clientLogger.info('state:', isSaccade.value, 'vel: ', ret, 'pos:', eye_position.value)
        return ret
    #
